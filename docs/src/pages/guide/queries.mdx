...
You can tag the queries with an array of strings. These tags have many functions. For now, they help with allowing you to clear the cache for some queries and also allow mutations to auto-clear the queries' cache that has any of the same tags.
...
To see how mutations can be configured to clear tagged queries, check the [mutation's guide](/guide/mutations#clearing-tagged-queries-cache).
